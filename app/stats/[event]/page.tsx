...
export default function Event({ params }: { params: { event: string } }) {
  ...
  useEffect(() => {
    appwriteConfig.databases
      .listDocuments(`${process.env.NEXT_PUBLIC_DATABASEID}`, params["event"])
      .then(
        function (response) {
    ...
    const unsubscribe = appwriteConfig.client.subscribe(
      `databases.${process.env.NEXT_PUBLIC_DATABASEID}.collections.${params["event"]}.documents`,
      (response) => {
        appwriteConfig.databases
          .listDocuments(`${process.env.NEXT_PUBLIC_DATABASEID}`, params["event"])
          .then(
  ...
  const handleAcceptanceEmail = (id: string, name: string, email: string) => {
    appwriteConfig.databases
      .updateDocument(`${process.env.NEXT_PUBLIC_DATABASEID}`, params["event"], id, {
        confirm: "accept",
      })
      .then(() => {
        callAPI(
